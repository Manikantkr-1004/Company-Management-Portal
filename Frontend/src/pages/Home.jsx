...
export default function Home() {
    ...
    return (
        ...
        className="w-full min-h-screen px-5 text-center flex flex-col justify-center items-center gap-5">
            ...
            {user.isLoggedIn && <Link to={'/dashboard'} className="bg-(--dark-color) shadow-white shadow-2xl text-white rounded-md text-sm font-medium px-5 py-2">Visit Dashboard</Link>}
            {!user.isLoggedIn && <Link to={'/auth/login'} className="bg-(--dark-color) shadow-white shadow-2xl text-white rounded-md text-sm font-medium px-5 py-2">Login</Link>}
        </section>
    );
}
